...
`make reproduce` runs `python reproduce.py`, which is the single entry point. It takes about 20 seconds and ends by reloading every saved model and replaying 135 forecasts. `make help` lists the other targets; `python reproduce.py --stages all` runs the whole chain including the 20 GB download.
...
Read those rankings with `results/seed_separation.csv` beside them. In 4 of 5 areas the top two models' RMSE ranges overlap across seeds, and only 1 area has a gap larger than the combined seed standard deviation. Validation and test also disagree about the winner in 5 of 5 areas. One week at one seed does not establish an ordering of these three architectures.
...
```sh
make tune     # the four validation rounds, on area 5161, seed 42
