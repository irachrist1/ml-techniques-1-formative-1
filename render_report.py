...
def render(source=None):
    ...
    while i<len(lines):
        ...
        if line.startswith('|'):
            ...
            elif 'Experiment' in table_lines[0]: widths=[(A4[0]-84)*v for v in [.24,.23,.18,.13,.22]]
            if len(widths) != len(rows[0]): raise ValueError('Table width count must match columns')
            if abs(sum(widths)-(A4[0]-84)) > 1e-6: raise ValueError('Table exceeds printable width')
