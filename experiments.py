...
def windows(frame, area, lookback):
    """Build past-only windows on the common 144-step eligibility mask.

    Every candidate sees the same target timestamps regardless of its own
    lookback, so a shorter history cannot win by being scored on more points.
    """
    ...
    if not np.all(np.diff(timestamps) == 600000):
...
def receptive_field(dilations, kernel=3):
...
def dataset(tf, d, seed=None, batch=128):
    ...
    if seed is not None:
        ds = ds.shuffle(len(d['y']), seed=seed, reshuffle_each_iteration=True)
...
def fit_one(kind, config, data, seed):
    ...
    if kind == 'RidgeAR':
        ...
        result = model.fit(dataset(tf, train, seed), validation_data=dataset(tf, validation),
...
def run(config_path, area, seed, phase):
    ...
    for kind, config in configs['models'].items():
        ...
        for split in (['validation'] if phase == 'tune' else ['validation', 'test']):
            d = data[split]
            correction = predict(d['x'])
            prediction = np.maximum(0, d['last'] + scaler['std'] * correction)
        ...
        for i in range(min(30, len(timing_data['y']))):
            start = time.perf_counter()
            predict(timing_data['x'][i:i + 1])
            point.append(time.perf_counter() - start)
        model_summary['inference'] = {
            'batch_n': len(timing_data['y']),
